...
3. **Classify (free)** — sort the pending rows using only the metadata the free `GET /models` already returned — no extra call, no tokens. A model the catalog prices as non-free (a per-token `pricing` rate, or an explicit not-free flag) is marked `paid_tier`; a model whose declared output isn't text (image/audio/embedding) is marked non-chat. This resolves most of the "listed upstream but is it a routable free chat model?" question for free. Providers whose catalog omits this metadata are left untouched.
...
- **Added 2026-09-06 (each verified against the provider's own pricing/FAQ):** **ModelScope** — permanent 2,000 req/day, DeepSeek-V4 / Qwen3.5-397B / GLM-5.2 / MiniMax-M3, *Alibaba real-name/ID verification*, no payment method exists on the service (so it can never charge). **GMI Cloud** — $0 70B endpoints (rotating list), no card needed, probe-first. **Hetzner Inference** — free while experimental, hard 429, *may require card/ID*, small Qwen only.
- **Rejected under the "never auto-charge on overage" rule:** **Vercel AI Gateway** — evaluated and dropped 2026-09-06. Signup forces a card and the account is pay-as-you-go; Vercel's docs confirm a free tier + auto-top-up-off-by-default but never state that a free-credit-exhausted request *refuses* rather than billing the card, and the free-model set (`freeTier=true`) could not be shown to equal the `-free` id suffix a gate would key on. Card-on-file + PAYG + an unverifiable free-set is the exact blast radius the rule forbids. Re-add only with a doc line proving a balance-zero free-tier call returns 402 instead of charging. **Held:** Public AI (prepaid starter credits of unverified size/renewal — trial-shaped). **Skipped as trials:** Alibaba Model Studio and Tencent TokenHub (90-day), BytePlus ModelArk (one-time). **Dead (no free tier any more):** Chutes, Kluster, Together, Targon, Featherless, glhf.

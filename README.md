...
**Hard provider anti-affinity.** A request from a *swarm consumer* (`FEEDER_SWARM_CONSUMERS`, default `ringer`) that carries a session id (`X-Session-Id` / `session_id`) is spread onto a platform **not** currently held by a sibling worker of the same consumer, and holds it for the job. The first call of each worker reserves its platform under a lock so simultaneous siblings can't race onto the same provider; the lane is refreshed on every call and released after `FEEDER_SWARM_LANE_IDLE_MS` of silence. If every platform is held by a sibling, routing returns `429 ALL_RATE_LIMITED` (transient — hold the worker and re-query capacity), never `422`. This stops N same-class workers converging on one provider and rate-limiting it, and preserves each worker's provider prompt-cache.
...
- **Web-search backend** is pluggable and catalog-driven. Built-in providers: **Tavily**, **Ollama Web Search**, **Brave**, **Serper**, **Exa** (keyed) and **DuckDuckGo** (keyless). **Manage them from the "SEARCH PROVIDERS" card** on both the Onboarding page and the Key Vault: paste a key (stored **encrypted in the DB**, one `search_key_<id>` row per provider — never in `.env`), **VERIFY** it with a single live query, and pick which backend is active. The catalog is `SEARCH_PROVIDER_CATALOG` (`server/src/services/searchConfig.ts`) — the single source of truth for both server and UI; add a provider there and implement its `SearchBackend` in `webSearch.ts` under the same id. DB config overrides the `.env` bootstrap fallback at boot/update via `loadSearchConfigIntoEnv`, live without a restart.

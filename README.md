...
- **Added 2026-09-06 (each verified against the provider's own pricing/FAQ):** **Vercel AI Gateway** — recurring $5/month credit, 9 `-free` models incl. `minimax/minimax-m3-free`, *card required*, hard-stops (402) unless Auto Top-Up is enabled (default off). **ModelScope** — permanent 2,000 req/day, DeepSeek-V4 / Qwen3.5-397B / GLM-5.2 / MiniMax-M3, *Alibaba real-name/ID verification*, no payment method exists. **GMI Cloud** — $0 70B endpoints (rotating list), no card needed, probe-first. **Hetzner Inference** — free while experimental, hard 429, *may require card/ID*, small Qwen only. **Held:** Public AI (prepaid starter credits of unverified size/renewal — trial-shaped). **Skipped as trials:** Alibaba Model Studio and Tencent TokenHub (90-day), BytePlus ModelArk (one-time). **Dead (no free tier any more):** Chutes, Kluster, Together, Targon, Featherless, glhf.

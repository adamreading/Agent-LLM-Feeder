...
# 3b. (Optional) Import the curated wiki seed — a real starting-point knowledge
#     base of 574 canonical model entries (542 with researched summaries, plus
#     modality flags, task scores and aliases) and 173 real provider instances
#     (measured capabilities, context windows, free-tier rate limits). Curated,
#     not a raw dump: unverified/dead catalog rows are excluded, so your own
#     keys + startup discovery repopulate the live instances. Idempotent,
#     natural-key keyed (canonical slug + platform/model_id — never serial ids),
#     no secrets, safe to re-run. (Regenerate any time with `npm run seed:wiki:export`.)
...
| `session_id` / `user` (or `X-Session-Id` header) | Sticky routing — keep one conversation/job on one model+provider across calls. Body `session_id`/`user` win; the `X-Session-Id` header is read as a last resort so an OpenCode client (which stamps it natively, per invocation) gets sticky pinning + per-session request attribution with zero plumbing. A pin lasts while the session stays active (30-min idle TTL, sliding) **and at most `FEEDER_STICKY_MAX_AGE_MS` (default 2 h)** — after that the next turn re-routes fresh and re-pins. The cap exists because a sticky session bypasses every spread mechanism below, and a bot that reuses **one** `session_id` for a whole room never goes idle: one such room sent 558 requests in a day to a single model (2026-09-11). If you run a room/channel bot, send a per-thread `session_id` rather than a room-wide one. |
...
3. **Rank** the survivors by `health × (quality + latency)`, where latency's weight scales with the declared `latency_ceiling_ms` (tight → speed dominates for chat; loose → quality dominates for batch). **Quality is the dominant term, and where it comes from matters** (fixed 2026-09-12): the prior is the model's per-task score from **external leaderboards** — LMArena human-preference Elo per category (arena.ai; the 8 categories are exactly feeder's task types) and, if you add a free key, Artificial Analysis's intelligence/coding/math indices — imported **weekly with zero model tokens** (`services/leaderboardSync.ts`; 8 page GETs + 1 API GET), normalised relative to the best model on each board. A model the boards don't cover falls back to the web-research writer's *estimate* (`source='research_estimate'`) at **a quarter of the weight** (`FEEDER_RESEARCH_PRIOR_CONFIDENCE`, default 0.25) — those estimates were the only prior until 2026-09-12, labelled "benchmark", and measured out at a −0.27 correlation with live success (a transcription model scored coding = 1.00 off a newsletter). Real-usage quality (`realtime_quality`, when a consumer reports it) blends over whichever prior exists. The curated intelligence rank is a small tiebreak; every auto-discovered model carries the default `#500` there, so don't read it as a ranking.
...
5. **Explore** (non-sticky, non-pinned requests only). A quality-fed ranker starves its own tail — a model can only earn a score by being routed to — so three mechanisms deliberately spend a little traffic outside the score: every **20th** request gives the single most-overdue live model first shot (tail slot); every **4th** request (`FEEDER_OR_FREE_EVERY`) gives the most-overdue healthy **OpenRouter `:free`** model first shot, skipping any whose known task score is below `FEEDER_OR_FREE_MIN_SCORE` (0.5; an unscored new model still gets one try); otherwise **15%** of requests (`ROUTE_EXPLORE_EPSILON`) take a random one of the top 6. All three still pass the hard filter in step 2, so nothing ineligible is ever served. ⚠️ Until 2026-09-11 all three were silently dead: their candidate filter read `fallback_config.enabled` (true on every auto-added row) instead of `models.enabled`, so "most overdue" was always a delisted model that the walk then skipped — the slots fired on schedule and changed nothing. The chain is now built from live rows only. Sticky sessions (a `session_id`, or a multi-turn conversation) bypass all three by design — see the `session_id` row above for the 2-hour cap that keeps a never-idle session from monopolising one model.
...
- **Web search is a load-balanced BANK, not one backend.** Instead of a single active engine (which rate-limits under a burst), search spreads across an activated **bank** of free engines — the same idea as the model router spreading across providers. `services/searchPool.ts` picks the **least-recently-used healthy free** engine per query (even spread so no one engine gets hammered), **cools down** any engine that throttles (429) so load shifts off it, tracks **per-engine latency/success + free-tier quota** (`search_backend_health`), and falls through to a **paid last-resort tier (You.com)** only when every free engine is exhausted. **Quota-aware:** each engine's best-effort free quota (catalog `freeQuota`) is tracked against a monthly usage window; an engine near its cap drops to a secondary band so its last credits are spared (a small tier like SerpApi's 100/mo isn't drained by the even spread), and an exhausted engine is skipped entirely. You.com is guarded by a **per-job ($5, keyed on `X-Run-Id`) and global (~$180) spend cap** so a paid credit can't run away. When nothing could be injected, the response carries `X-Augment-Skipped` (`throttled`/`no-results`/`no-config`/`error`) — see the augment section.
- **Catalog-driven, extensible.** Built-in engines: **Tavily**, **Ollama Web Search**, **Brave**, **Serper**, **Exa**, **SerpApi**, **TinyFish** (free 30 q/min), **ContextWire** (1k/mo), **Scavio** (250/mo), **SearXNG** (self-hosted, unlimited — the "key" is your instance base URL) — all keyed/free for the spread — plus **DuckDuckGo** (keyless) and **You.com** (paid last-resort). **Manage them from the "SEARCH PROVIDERS" card** on both Onboarding and the Key Vault: paste a key (stored **encrypted in the DB**, one `search_key_<id>` row per provider — never in `.env`), **VERIFY** it with one live query, then **toggle each engine into the bank** (per-engine live latency/success + You.com spend are shown on the card). The catalog is `SEARCH_PROVIDER_CATALOG` (`server/src/services/searchConfig.ts`) — the single source of truth for server + UI; add a provider there and implement its `SearchBackend` in `webSearch.ts` under the same id. DB config overrides the `.env` bootstrap fallback at boot/update via `loadSearchConfigIntoEnv`, live without a restart.
- **Writer model** is `RESEARCH_MODEL` (pick one strong at writing) or auto-picked.
- **Grounded, not fabricated — but still an estimate.** The writer uses only the fetched sources and nulls any score it can't support. Its per-task scores are stored as `source='research_estimate'` (before 2026-09-12 they were labelled `benchmark`, which overstated them: they are an LLM's reading of search snippets, and they measured out at a −0.27 correlation with live success). The router uses them only as a **weak fallback** (a quarter of the normal quality weight) for models the leaderboards don't cover.
- **Quality priors come from leaderboards, for free.** `services/leaderboardSync.ts` imports **LMArena** per-category Elo (arena.ai — 8 page GETs, no key) and, if `ARTIFICIAL_ANALYSIS_API_KEY` is set, **Artificial Analysis**'s intelligence / coding / math indices (free API, 1,000 req/day, [attribution required](https://artificialanalysis.ai/)). Zero model tokens; runs **weekly** in-process (`FEEDER_LEADERBOARD_PERIOD_HOURS`, default 168), or on demand with `npm run leaderboard` / `POST /api/catalog/leaderboard` (localhost). Scores are normalised relative to the best model on each board and matched to canonical models by exact normalised name only (reasoning-effort variants like `-high`/`-thinking` collapse onto the base model at their median; anything ambiguous is refused, never guessed). `GET /api/catalog/leaderboard-status` shows the last run, including the board names that matched nothing.
- **Model sizes (`params_b`) are backfilled free too**, in the same weekly run (`services/paramsBackfill.ts`). First the size token already in the id/name (`…-550b-a55b` → 550, ignoring the active-params `a55b`; `8x22b` → 176; a bare expert count like `-128e` it can't total → skipped), then, for the rest, HuggingFace's `safetensors.total` (search → exact-name repo → param count, bounded to `FEEDER_PARAMS_HF_LIMIT` routable models/run). `params_b` drives the quality-lift **size tilt** and the **big100 band** (`>= 100B` or a curated `frontier` flag). Undisclosed-size proprietary models (Gemini, Mistral Medium, Cohere Command) stay null and qualify for big100 only via the `frontier` flag. New models get the free token-parse at discovery, same-day.
...
- **Hot-path auto-bench on the first real failure (zero extra tokens).** A live request that fails with a *persistent* error benches the model immediately, so it stops leading and failing over on every call: a **payment wall** (HTTP 402, "payment method required", "unavailable for free — paid version available") → `disabled_reason='paid_tier'` **and** `cost_tier='paid'` so it can never route as free again; **403/404/410 / "gone" / decommissioned / model-not-found** → `unreachable`. A request-shaped 400 never benches a healthy model. (Before 2026-09-06 only 403/404 were caught, so providers whose free tier had ended — SambaNova 402, Cerebras 402, GitHub Models 410 — sat at 0% success for two weeks while still enabled.)
- **Long-window low-success bench.** The 30-minute health window needs 8 attempts *inside one window* to bench a low-success model, so a model that gets a handful of tries a day and fails every one never tripped it. A second pass over **7 days** benches any enabled model with ≥5 attempts and <15% success (`low_success`, daily-revived). Tunable: `FEEDER_LONG_LOW_SUCCESS_MIN_ATTEMPTS` (default 5).
- **Feeder's own network failures don't score against a model.** `fetch failed` / DNS / connection-refused are the host's egress blipping (measured: bursty, each burst hitting 7–9 providers at once — ~46% of all logged failures, including against providers at 96% success). They stay in the request log but are excluded from health, success rate, cooldowns and both benches, so a good model isn't demoted for the box's network. The proxy also **retries the same route once after 400ms** on a network-class failure (one retry per request, no cooldown, not counted against the model) — 67% of those failures landed in a minute that also had successes, i.e. sub-minute blips a single retry recovers; a genuine outage costs ~400ms plus one fast failure before normal failover.
...
3. **Classify (free)** — sort the pending rows using only the metadata the free `GET /models` already returned — no extra call, no tokens. A model whose `pricing` is anything other than a flat zero is marked `paid_tier` — that means a positive per-token rate **and** OpenRouter's `-1` sentinel for a **variable-priced** meta/router model (`openrouter/auto`, `auto-beta`, `fusion`, …), which charges whatever the model it routes to costs and is therefore not free; an explicit not-free flag (kilo `isFree:false`) counts too. A model whose declared output isn't text (image/audio/embedding) is marked non-chat. Beyond the pending backlog, this also demotes any **already-enabled** free-tier row the catalog now prices — so a router that once slipped through as free can't keep taking paid traffic (a manual/no-key/unhealthy row is never touched). Providers whose catalog omits this metadata are left untouched.
3b. **Enable-on-discovery (default since 2026-09-06, zero-token)** — every remaining pending free **chat** model on a platform we hold a usable key for goes straight to `enabled=true`. No liveness completion is spent to "prove" it; reliability is enforced **observe-on-use** instead — the hot-path benches (payment wall → `paid_tier`, 403/404/410 → `unreachable`) fire on the first real failure and the 30-min + 7-day health passes catch the rest, so the total cost of a dead model is one failed attempt (then fallback), once. This is what lets a newly-listed free model be routable the same day it appears upstream instead of queueing behind a ≤15/day probe cap. Set `FEEDER_ENABLE_WITHOUT_PROBE=0` to restore the old probe-first path.
3c. **Fallback rows** — every model row gets its `fallback_config` entry here (idempotent). Before 2026-09-11 those rows were only created at boot, so a model this sync added and enabled stayed **invisible to the router until the next restart**.
3d. **Kind re-classify (zero-token)** — the id heuristic (`modelKind.ts`: embedding / tts / rerank / ner / image-gen / **audio-gen** / moderation) is re-run over every `kind='chat'` row, not only at insert. A flip to non-chat sets `kind` (a structural exclusion from routing) and disables the row with a reason. Added after Google's **Lyria music models** sat as chat on two providers — OpenRouter lists them with `output_modalities ["text","audio"]` and pricing `0/0`, so step 3's metadata backstop passed them — and a creative-task caller was routed to a music generator 147 times.
4. **Retire (soft)** — for a provider that polled cleanly (HTTP 200, non-empty list), an id previously seen live but now absent for **3 consecutive polls** is soft-retired (`enabled=false, disabled_reason='delisted'`). The run summary's `retired` figure counts **newly** retired rows (before 2026-09-11 it re-stamped every already-delisted row and reported the same ~150 "retired" on every run — alarming, but nothing routable had changed). The counter is per *run*, not per day — normally one run a day, but a manually-triggered `POST /api/catalog/sync` counts too, so triggering it several times in a day retires absent ids sooner (they were absent from the provider's live list on every poll, so nothing routable is lost). A failed or empty poll retires **nothing** (a provider blip can't delist a model), a manually-disabled or otherwise-benched row is never overridden, and a model that reappears upstream is un-retired automatically.
5. **Canonicalise** — new chat models still awaiting liveness are linked/grouped and get a wiki entry (models already sorted to paid/non-chat in step 3 are skipped, so no research is spent on them).
6. **Enable (leftovers)** — a **bounded** liveness pass (≤15/run, one small real call each) over whatever is still pending. With enable-on-discovery on, that is only models on platforms with **no** usable key, so this stage normally spends nothing; paid/dead ones stay disabled with an honest reason.
7. **Research** — a **bounded** pass (≤10/run) writes wiki summaries for the new canonical models.

Soft-retire only: the row, its researched summary, and its slug/links are preserved, and a delisted model returns to the wiki if it reappears upstream. Because a retired model is simply `enabled=false`, the wiki (which lists only served instances) drops it with no extra filtering. Discovery, classification and enable-on-discovery are free; only the leftover-liveness and research stages (6, 7) can spend tokens, and both are capped. The boot-time capability probe over never-measured enabled models (`autoOnboard` → `probeNeverProbed`, tools + json_mode, 2 small completions per model) is capped too — `FEEDER_ONBOARD_PROBE_LIMIT`, default 20 models per boot — because enable-on-discovery can hand it a backlog of hundreds at once (one boot fired 264 probes on 2026-09-06 before the cap existed); the backlog drains a slice per boot.
...
npm run leaderboard  # ZERO model tokens: import external quality priors now (LMArena
                     # arena.ai pages + Artificial Analysis if ARTIFICIAL_ANALYSIS_API_KEY is
                     # set) into task_scores. Runs weekly on its own; this forces it.
```
...
- **Inclusion rule for new providers (Adam, 2026-09-06):** the free tier must **hard-stop** when exhausted — never auto-charge on overage — and no 90-day or one-time trials. Any card / ID / phone requirement is stated on the provider's card in the UI (the amber ⚠ line). On gateways that list paid and free models together without machine-readable pricing, only ids matching the free convention are ever enabled or probed (`catalogSync` `MIXED_GATEWAY_FREE_ID`: opencode and Vercel `-free`); everything else is `paid_tier` at discovery, so a card on file can never be touched by a probe.
- **Added 2026-09-06 (each verified against the provider's own pricing/FAQ):** **GMI Cloud** — $0 70B endpoints (rotating list), no card needed, probe-first. **Hetzner Inference** — free while experimental, hard 429, *may require card/ID*, small Qwen only.
- **Rejected 2026-09-06:** **Vercel AI Gateway** — signup forces a card and the account is pay-as-you-go; Vercel's docs confirm a free tier + auto-top-up-off-by-default but never state that a free-credit-exhausted request *refuses* rather than billing the card, and the free-model set (`freeTier=true`) could not be shown to equal the `-free` id suffix a gate would key on. Card-on-file + PAYG + an unverifiable free-set is the exact blast radius the "never auto-charge on overage" rule forbids. **ModelScope** — API-Inference returns 401 "bind your Alibaba Cloud account" on every completion (the key lists models fine, which masks it in a GET-based health check), and that binding only accepts a China-region Alibaba Cloud account — an international one will not bind, so it's unusable for a non-China user. **Held:** Public AI (prepaid starter credits of unverified size/renewal — trial-shaped). **Skipped as trials:** Alibaba Model Studio and Tencent TokenHub (90-day), BytePlus ModelArk (one-time). **Dead (no free tier any more):** Chutes, Kluster, Together, Targon, Featherless, glhf.

...
// directive): an external caller (Hermes, Open Brain's own tooling) reports
...
// consumer-specific probes (ob_readwrite hitting Open Brain's REST API)
...
describe('Generic capability-reporting API', () => {
  ...
  it('records a new capability fact for a real catalog model', async () => {
    ...
    const { status, body } = await request(app, 'POST', '/api/capabilities', {
      ...
      capability: 'ob_readwrite',
      supported: true,
      evidence: 'external probe run by Hermes against Open Brain',
    });

    expect(status).toBe(201);
    expect(body).toMatchObject({ platform: model!.platform, model_id: model!.model_id, capability: 'ob_readwrite', supported: true });
    ...
    const row = await get<{ supported: boolean; source: string }>(getPool(), `
      ...
      WHERE m.platform = ? AND m.model_id = ? AND mc.capability = 'ob_readwrite'
  ...
  it('upserts (does not duplicate) when reported again for the same model+capability', async () => {
    ...
    await request(app, 'POST', '/api/capabilities', { platform: model!.platform, model_id: model!.model_id, capability: 'ob_readwrite', supported: false });
    await request(app, 'POST', '/api/capabilities', { platform: model!.platform, model_id: model!.model_id, capability: 'ob_readwrite', supported: true });

    const rows = await getPool().query(`
      SELECT mc.supported FROM model_capabilities mc
      JOIN models m ON m.id = mc.model_db_id
      WHERE m.platform = $1 AND m.model_id = $2 AND mc.capability = 'ob_readwrite'
    `, [model!.platform, model!.model_id]);
  ...
  it('GET returns reported capabilities for a model', async () => {
    ...
    expect(body.capabilities).toContainEqual(expect.objectContaining({ capability: 'ob_readwrite', supported: true }));
  });
});

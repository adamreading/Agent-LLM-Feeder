...
import { getSearchPool, isPaidBackend } from './searchConfig.js';
...
async function recordAttempt(backend: string, ok: boolean, latencyMs: number, opts: { error?: string; throttled?: boolean } = {}): Promise<void> {
  ...
  if (ok) {
    await run(getPool(), `
      INSERT INTO search_backend_health (backend, recent_latency_ms, success_count, calls_total, consecutive_failures, cooldown_until, last_used_at, updated_at)
    ...
    await run(getPool(), `
      INSERT INTO search_backend_health (backend, fail_count, calls_total, consecutive_failures, cooldown_until, last_error, last_used_at, updated_at)
      VALUES (?, 1, 1, 1, ?, ?, ?, now())
      ON CONFLICT (backend) DO UPDATE SET
...
export async function poolSearch(query: string, maxResults = 6, opts: { runId?: string | null } = {}): Promise<PoolSearchResult> {
  ...
  const freeReady = lruSort(freeConfigured.filter((id) => !cooled(health.get(id), nowMs)), health);
...
/** Per-engine telemetry for the UI (adds est spend + cap for paid engines). */
export async function getSearchHealth(): Promise<Array<HealthRow & { paid: boolean; estSpendUsd: number | null }>> {
  const health = await loadHealth();
  const pool = await getSearchPool(getPool());
  const ids = Array.from(new Set([...pool, ...health.keys()]));
  return ids.map((id) => {
    const h = health.get(id) ?? { backend: id, recent_latency_ms: null, success_count: 0, fail_count: 0, consecutive_failures: 0, calls_total: 0, cooldown_until: null, last_error: null, last_used_at: null };
    const paid = isPaidBackend(id);
    return { ...h, paid, estSpendUsd: paid ? Number((h.calls_total * YOU_COST_PER_CALL).toFixed(2)) : null };
  });
}
